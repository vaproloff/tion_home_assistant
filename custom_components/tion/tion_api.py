...
class TionConnection:
    def __init__(self, data: dict):
        ...
        self.last_packet_time_iso = data.get(
            "last_packet_time_iso"
        )  # 2019-03-09T09:16:50.2467772Z
...
class TionZonesSchedule:
    def __init__(self, data: dict):
        ...
        self.current_preset = TionZonesScheduleCurrentPreset(
            data.get("current_preset", {})
        )
        self.next_preset_starts_at = data.get("next_preset_starts_at")  # 1552176300
        self.next_starts_iso = data.get(
            "next_starts_iso"
        )  # 2019-03-10T00:05:00.0000000Z
...
class TionZonesSensorsAverageData:
    def __init__(self, data: dict):
        ...
        self.measurement_time_iso = data.get(
            "measurement_time_iso"
        )  # 2019-03-09T09:16:52.0000000
...
class TionZones:
    def __init__(self, data: dict):
        ...
        self.creation_time_iso = data.get(
            "creation_time_iso"
        )  # 2019-03-05T10:27:31.7207231Z
        self.creation_time = data.get("creation_time")  # 1551781651
        self.update_time_iso = data.get(
            "update_time_iso"
        )  # 2019-03-05T10:27:31.7207231Z
...
class TionApi:
            ...
            if response.status_code == 200:
                ...
                if self._auth_fname:
                    try:
                        ...
                        _LOGGER.error(
                            f"Unable to write authorization data to {self._auth_fname}: {e}!"
                        )
                _LOGGER.info("Got new token")
                return True
            else:  # pragma: no cover
                _LOGGER.error(
                    f"Status code while getting token: {response.status_code}, content:\n{response.json()}!"
                )
                return False
    ...
    def wait_for_task(self, task_id: str, max_time: int = 5) -> bool:
        ...
        for cnt in range(int(max_time / DELAY)):
            ...
            if response.status_code == 200:
                ...
                _LOGGER.warning(
                    f"Bad response code {response.status_code} in wait_for_task, content:\n{response.text}"
                )
                return False
        _LOGGER.warning(
            f"Couldn't get completed status for {max_time}sec in wait_for_task"
        )  # pragma: no cover
        return False  # pragma: no cover

    def get_data(self, force=False) -> bool:
        if (
            not force and (time() - self._last_update) < self._min_update_interval
        ):  # update only once per {min_update_interval} seconds
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if response.status_code == 401:
                    ...
                    _LOGGER.debug(
                        f"Status code while getting data is {response.status_code}, content:\n{response.json()}!"
                    )
                    return False
        except requests.exceptions.RequestException as e:  # pragma: no cover
    ...
    def _get_zones_data(
        self, name_part: str = None, guid: str = None, force=False
    ) -> list:
        result = []
    ...
    def _get_devices_data(
        self, name_part: str = None, guid: str = None, type: str = None, force=False
    ) -> list:
        devices_data = []
        zones = []
        if self.get_data(force=force):
    ...
    def get_devices(
        self, name_part: str = None, guid: str = None, type: str = None
    ) -> list:
        ...
        for device_data, zone in zip(devices_data, zones):
            if "co2" in device_data.type:
                ...
                _LOGGER.error(
                    f"Unknown device type: {device_data.type}, contact the developer for support with data:\n{device_data}"
                )
        return result
...
class Zone:
    ...
    def send(self) -> bool:
        ...
        try:
            ...
            _LOGGER.error(
                f"Exception while sending new zone data!:\n{e}"
            )  # pragma: no cover
            return False  # pragma: no cover
        js = js.json()
        status = js["status"]
        if status != "queued":
            _LOGGER.info(
                "TionApi auto set " + status + ": " + js["description"]
            )  # pragma: no cover
            return False  # pragma: no cover
        else:
...
class Breezer(TionZonesDevices):
    ...
    def send(self, extra_data: dict = None) -> bool:
        ...
        heater_enabled = (
            bool(self.heater_enabled) if self.heater_enabled is not None else False
        )
        data = {
            "is_on": True if speed > 0 else False,
            "heater_enabled": bool(self.heater_enabled)
            if self.heater_enabled is not None
            else False,
            "heater_mode": "heat"
            if heater_enabled
            else "maintenance",  # 4S model support
            ...
            if self.speed_min_set is not None
            else 0,
            "speed_max_set": int(self.speed_max_set + 0.5)
            if self.speed_max_set is not None
            else 6,
        }
        ...
        try:
            ...
            _LOGGER.error(
                f"Exception while sending new breezer data!:\n{e}"
            )  # pragma: no cover
            return False  # pragma: no cover
        js = js.json()
        status = js["status"]
        if status != "queued":
            _LOGGER.error(
                "TionApi parameters set " + status + ": " + js["description"]
            )  # pragma: no cover
            return False  # pragma: no cover
        else:
            return self._api.wait_for_task(js["task_id"])
    ...
    def load(self, device_data: TionZonesDevices = None, force=False):
        ...
        if device_data:
            ...
            if (
                self._heater_installed is None and "4S" in self._name
            ):  # 4S does not give that for some reason
            ...
            self.speed = (
                data.speed if self._is_on else 0
            )  # Tion gives speed 1 even if it's off
