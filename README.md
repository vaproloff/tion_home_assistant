...
Она добавляет управление бризерами Tion Breezer O2/3S/4S, станцией MagicAir и модулем CO2+, а также датчики температуры, CO2, влажности, PM2.5 и состояния фильтров.
...
- Управление бризером как `climate`: включение, выключение, нагрев, скорость вентилятора и источник воздушного потока для поддерживаемых моделей.
- Датчики температуры входящего и выходящего потока воздуха.
- Управление подогревом, подсветкой и звуковыми сигналами для поддерживаемых моделей.
- Настройка минимальной и максимальной скорости для Авто режима.
- Дата следующей замены фильтров, индикатор необходимости замены и кнопка сброса периода замены для 3S/4S.
...
- `climate` - управление бризером:
  ...
  - `fan_mode`: `1`-`max_speed`, `auto`;
  - `swing_mode`: `outside`, `inside`, `mixed` для 3S/4S;
...
- `button` - сброс периода замены фильтров для 3S/4S.
- `switch` - подогрев воздуха.
- `switch` - подсветка кнопки для 3S/4S.
- `switch` - звуковые сигналы для 3S/4S.
...
- `climate.set_swing_mode` - выбор источника воздушного потока для поддерживаемых моделей.
- `number.set_value` - настройка целевого CO2 и скоростей Авто режима.
- `switch.turn_on`, `switch.turn_off`, `switch.toggle` - управление подогревом, подсветкой и звуком для поддерживаемых моделей.
- `button.press` - сброс периода замены фильтров для 3S/4S через кнопку `button.<device>_reset_filters`.
