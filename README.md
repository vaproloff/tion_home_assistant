...
This is a custom HomeAssistant integration for Tion Breezer 3S/4S, MagicAir and Module CO2+ devices. It uses the Tion/MagicAir cloud API, exposes native HomeAssistant entities, supports setup through the UI, offers per-breezer speed presets, and can run local CO2 PID control from an external HomeAssistant sensor.
...
- Сохранять профили границ скорости как пресеты и переключать их одним выбором.
...
Пресеты позволяют сохранить несколько профилей границ скорости Авто-режима зоны (`speed_min_set` / `speed_max_set`) и переключаться между ними одним выбором в карточке бризера. Удобно для типовых сценариев: тихий ночной режим, экономичный режим днём или проветривание на максимуме.

Пресет хранит пару значений — минимальную и максимальную скорость. При активации пресета интеграция выставляет эти границы бризеру, а при возврате к `Без пресета` восстанавливает те границы, что стояли до включения пресета. Имена пресетов берутся из стандартного списка HomeAssistant (`Eco`, `Away`, `Boost`, `Comfort`, `Sleep`, `Activity`, `Home`), поэтому frontend сам подставляет родные переводы и иконки. Набор пресетов настраивается отдельно для каждого бризера.
...
5. Для `Add preset` выберите имя пресета, затем задайте минимальную и максимальную скорость.
...
Минимальная скорость не может быть больше максимальной — иначе форма покажет ошибку. Диапазон ограничен максимальной скоростью конкретного бризера. После настройки у бризера в `climate` появляется выбор пресета (`preset_mode`).

### Особенности

- Активация пресета меняет только границы скорости и не переводит зону в Авто-режим — пресет влияет на бризер, когда зона уже работает в Авто.
- Ручное изменение минимальной или максимальной скорости при активном пресете (через `number`-сущности `Min Speed Set` / `Max Speed Set` или приложение Tion) сбрасывает пресет в `Без пресета`. Так же ведёт себя большинство climate-интеграций.
- Активный пресет и сохранённые «допресетные» границы переживают перезапуск HomeAssistant.
- Пресеты и локальный PID независимы: PID управляет фактической скоростью бризера в ручном режиме, пресеты задают границы Авто-режима зоны.
- Если отредактировать границы уже активного пресета, на ближайшем обновлении облако ещё сообщит прежние границы, и пресет сбросится в `Без пресета`. Чтобы применить новые значения, выберите пресет заново.
